import { Blockhash, Keypair, LAMPORTS_PER_SOL, PublicKey, SystemProgram, TransactionInstruction, TransactionMessage, VersionedTransaction } from "@solana/web3.js";
import { connection, tipAcct, wallet } from "../config";
...
async function distributeWSOL(jitoTip: number, steps = 5) {
    ...
    for (let i = 0; i < Math.min(steps, keypairs.length); i++) {
        ...
        ixsTransfer.push(transferIx);

        ixsTransfer.push(createSyncNativeInstruction(
            ataAddressKeypair
        ))
    ...
    const chunkSize = 10; // EDIT CHUNK SIZE
